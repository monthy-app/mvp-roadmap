...
4. **Kick off.** Produce the execution plan:
   ...
   - the acceptance criteria — 3–7 testable one-liners defining done from the outside ("user can X", "input Y produces Z", "error path fails with W"), each *derived* from a sentence in the linked PRD section. An acceptance criterion with no spec sentence behind it means the section has a gap — grill it, never invent it;
   - the benchmarks, when the epic's feature touches a metric the PRD's validation strategy names: the command, the metric, and its budget — run at epic completion so numbers land early and become the baseline later epics regress against (omit when nothing measurable ships);
   - the regression checks the epic exits with — automated tests written as part of the epic, in the framework most appropriate to the task: one layer per acceptance criterion covering its edge cases, then the feature end-to-end as a whole, so added features can't silently regress it. If the right framework isn't installed, don't settle — ask permission to install it so the tests can be completed. The suite is built with `/regression`, composing with a TDD skill where one drives development;
