...
3. **Build the throwaway minimum that produces real numbers.** The smallest harness that makes the candidates comparable on the named metrics — a benchmark, a probe, a disposable prototype. Resist making it shippable; polish is scope stolen from the answer. With multiple candidates, hand the measurement down via `/delegate` — one subagent per candidate, each given the *same* harness and metrics spec so the numbers stay comparable; the timebox buys wall-clock from parallelism. The matrix assembly and the decision never delegate.
