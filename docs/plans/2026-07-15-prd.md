...
> Dated: 2026-07-15 · Living roadmap: [`ROADMAP.md`](../../ROADMAP.md) · Status: v0.4 — all scheduled epics (E1–E13) shipped
...
| Claude Code plugin marketplace | human dev | `/plugin marketplace add oxmonty/product-engineering-skills` then `/plugin install` delivers the whole collection as one managed bundle |
...
3. **Transcript review** (tertiary, human) — per golden brief, one recorded transcript reviewed against the interview rules: one decision per turn, recommendation always offered, brief context printed above every question, no survey-answerable questions.
...
### mvp (entrypoint)
The collection's front door, in two modes. Create mode is the phased interview specified in `references/decision-tree.md`: ten phases in dependency order, positioning-clause gate, propose-don't-ask trap elicitation, roadmap synthesized last with artifact-first epics — pre-MVP epics exit in a shippable artifact (pushed repo, published package, live URL, cut release), post-MVP epics name their feedback loop, E1 built core-primitive-first and shipping through one channel — and one fixed output shape regardless of project size: the `ROADMAP.md` index plus a dated PRD plus one epic file per epic under `docs/epics/`, so stories live in the same place in every project. On a repo with working features, create mode opens with a feature inventory instead: the baseline is recorded as already-true specification, E1 becomes characterization coverage of it, and a single-feature request is pushed back into a full roadmap. It closes by printing the journey: the companion command to run at each later stage. Update mode is anti-accretive editing per `references/editing-rules.md`: prime directive (rewrite as if written correctly from the beginning), edit propagation across section → linked stories → echoes, propose-then-tick with executed evidence, checkbox-only tick diffs; `/wrap-up` is its user-invoked face.
...
### spike
`/spike` buys a decision, not a feature: name the one question and the decision it blocks (an Open Questions entry), set a timebox and the metrics that settle it, build the throwaway minimum that produces real numbers (measurement fans out via `/delegate` — one subagent per candidate on an identical harness spec; matrix assembly and the decision never delegate), and record a decision matrix — options against measured criteria. The decision folds back per the editing rules (answer recorded on the question, owning section rewritten as if always true); the code is discarded by default — kept only if the user promotes it to real, reviewed code.
...
### Scope
**In scope:** the seven skills (mvp, wrap-up, epic, demo-ideas, delegate, spike, regression), the doc linter, the eval suites, three install channels (skills.sh, `.skill` releases, plugin marketplace).
**Future work (considered, not scheduled):** CI-automated evals (manual runs are cheap at this scale).
...
│   ├── wrap-up/                 # session close: SKILL.md + assets/write-up-template.md
│   ├── epic/                    # epic kickoff: SKILL.md + assets/epic-template.md
│   ├── demo-ideas/              # demos of current state
│   ├── delegate/                # tiered delegation
│   ├── spike/                   # one design question settled with measured evidence
│   └── regression/              # feature-pinning suites that gate epic ticks
...
├── docs/epics/                  # one file per epic (E<nn>-slug.md): stories, status, spec link; ROADMAP.md is the thin index
...
Channels: skills.sh (public repo with skills under `skills/` is the discoverable shape — the same layout mattpocock and superpowers ship), GitHub Releases carrying one packaged `.skill` bundle per skill for claude.ai import, and a Claude Code plugin marketplace hosted by this repo (`.claude-plugin/`): one bundled plugin carrying all seven skills — the loop is designed to be used together, so the managed channel installs it whole rather than à la carte. Release automation: tag push → package each `skills/<name>/` → attach artifacts.
