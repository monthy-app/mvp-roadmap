...
One file per epic, named `E<nn>-<slug>.md` (zero-padded so listings sort in epic order), with a fixed structure: the `# E<n>: Name` title, the outcome line with its link into the PRD section that specifies it, `## Stories` with their tick state, and — once `/epic` has run — `## Acceptance criteria` (3–7 testable one-liners defining done from the outside, each derived from the linked PRD section; `/regression` builds one test layer per item) and `## Exit criteria`: the artifact the epic exits with, the regression command that proves the acceptance criteria and gates the tick, the demo that proves it done, and — optional — the benchmark command and budget when the feature touches a measurable metric. Ticks live here — nowhere else — so a tick's blast radius is one epic's file.
